...
int create_sBk(uint32_t bSize, uint32_t nb_inode, unsigned int disk_size, T_SpBlk* dest);	/* create a new super block in dest
								  				   bSize is Block size in Byte
								   				   nb_inode is the number of inode to create
												   disk_size is in Bytes
												   Return 0 if sucess -1 else
						 	        				*/
int load_sBk(T_SpBlk* dest); 		//Load the super block in dest Return 0 if sucess -1 else
